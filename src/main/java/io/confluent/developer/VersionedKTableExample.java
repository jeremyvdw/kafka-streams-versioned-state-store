...
public class VersionedKTableExample {
    ...
    public Topology buildTopology(Properties allProps) {
        ...
        final VersionedBytesStoreSupplier versionedStoreSupplier2 = Stores.persistentVersionedKeyValueStore("versioned-ktable-store-2", Duration.ofMinutes(10));
        ...
        final KTable<String, String> streamInput = builder.stream(streamInputTopic, Consumed.with(stringSerde, stringSerde))
          .toTable(Materialized.<String, String>as(versionedStoreSupplier2)
              .withKeySerde(stringSerde)
              .withValueSerde(stringSerde));

        final KTable<String, String> tableInput = builder.stream(tableInputTopic, Consumed.with(stringSerde, stringSerde))
          .toTable(Materialized.<String, String>as(versionedStoreSupplier)
              .withKeySerde(stringSerde)
              .withValueSerde(stringSerde));
                        ...
                        for (int i = 0; i < list.size(); i++) {
                            ...
                            producer.send(new ProducerRecord<>(topic, 0, timestamp, key, value), (metadata, exception) -> {
                                if (exception != null) {
                                    ...
                                    System.out.printf("Produced record '%s - %s' at offset %d with timestamp %d to topic %s %n", key, value, metadata.offset(), metadata.timestamp(), metadata.topic());
                                }
                            });
                        }
                    }
